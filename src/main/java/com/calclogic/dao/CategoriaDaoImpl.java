...
public class CategoriaDaoImpl implements CategoriaDAO {
    ...
    public List<Categoria> getAllCategoriasByTeoria(Teoria teoria){
        //return this.sessionFactory.getCurrentSession().createQuery("FROM Categoria order by id").list();
        //return this.sessionFactory.getCurrentSession().createQuery("FROM Categoria WHERE teoriaid = :teoriaId OR EXISTS (SELECT padre FROM Incluye WHERE padre = teoriaid AND hijo = :teoriaId) OR EXISTS (SELECT padre FROM Incluye as i WHERE padre = teoriaid AND EXISTS (SELECT padre FROM Incluye WHERE padre = i.hijo AND hijo = :teoriaId)) order by id").setParameter("teoriaId", teoria.getId()).list();
