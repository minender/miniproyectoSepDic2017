...
public class TransitivityMethodImpl extends StartingOneSideMethodImpl implements TransitivityMethod {
                ...
                ResuelveManager resuelveManager, DisponeManager disponeManager, SimboloManager s)
    {
        ...
        boolean reversed = solved && typedTerm instanceof TypedApp && ((TypedApp)typedTerm).inferType=='e' &&
                      isInverseImpl(typedTerm.des("2", true).type().setToPrint(simboloManager),
                                                                         typedTerm.type().setToPrint(simboloManager));
        iter = (reversed ? typedTerm.des("2", true) : typedTerm);
        boolean lastEquan = solved && iter instanceof TypedApp && ((TypedApp)iter).inferType=='e' &&
                      iter.des("2", true).type().setToPrint(simboloManager).toString().equals("c_{8}");

        iter = (lastEquan ? iter.des("1", true) : iter);
        int i = 0;
        ...
        while (iter!=ultInf){
            ...
            if (iter instanceof TypedApp && ((TypedApp)iter).inferType=='t') {
               ...
               iter = iter.des("1", true);   
                     ...
                     iter.des("1", true) instanceof TypedApp && 
                     ((TypedApp)iter.des("1", true)).inferType=='m' && 
                     iter.des("11", true) instanceof TypedApp &&
                     ((TypedApp)iter.des("11", true)).inferType=='i'
                    ) { // iter is of the form ((IA)T)T wit modus pones
               ultInf = iter.des("2", true);
               iter = iter.des("12", true);
            }
            ...
            if (ultInfType instanceof App && ultInfType.des("1") instanceof App &&
                   !(ultInfType.des("11").toString().equals("c_{1}") ||
                     ultInfType.des("11").toString().equals("c_{15}")
                    )
               )
            {
                ...
                primExp = ultInfType.des("212").toStringLaTeX(s,""); 
        ...
        if ( firstOpInf == 0 || i == 1){
            Term last = reversed ? typedTerm.type().setToPrint(simboloManager).des("2") :
                                    typedTerm.type().setToPrint(simboloManager).des("12");
            lastline = solved ? last.toStringLaTeX(s,"")+"$":last.toStringLaTeXLabeled(s);
        }
        else {
            Term last = (reversed ? typedTerm.des("2", true) : typedTerm);
            last = (lastEquan?last.des("1", true):last).type().setToPrint(simboloManager).des("1212");;
            lastline = (solved?last.toStringLaTeX(s,"")+"$":last.toStringLaTeXLabeled(s));
        }
    ...
    private String stepTransOpInfer(String user, Term typedTerm, ResuelveManager resuelveManager, DisponeManager disponeManager, SimboloManager s) {
        ...
        while (iter != ultInf) {
            ...
            if (iter instanceof TypedApp && iter.des("1", true) instanceof TypedApp) {
                boolean modusPonens = ((TypedApp)iter).inferType=='m';
                ultInf = iter.des("1", true);
                iter = iter.des("2", true);
                
                Term t;
                if ( modusPonens ) {
                    App aux1 = (App)ultInf.des("2").type().setToPrint(simboloManager).des("122");
                    App aux2 = (App)ultInf.type().setToPrint(simboloManager).des("122");
                    if ( ((Var)aux1.q).indice == 112 ) {
                        t = new App(aux2.des("1"),new Var('z'));
                    }
                    else {
                        t = new App(new App(aux2.des("11"),new Var('z')), aux2.des("2"));
                    }
                }
                else {
        ...
        step = typedTerm.type().setToPrint(simboloManager).des("11").toStringLaTeX(s,"")+step.substring(step.indexOf("~"));
    ...
    private String stepTransEqInfer(String user, Term ultInf, ResuelveManager resuelveManager, DisponeManager disponeManager, SimboloManager s) {
        Term aux = ultInf.des("2", true);
        int z = ((Bracket)ultInf.des("1", true).type()).x();
        Term t = ((Bracket)ultInf.des("1", true).type()).t.des("12");
        Term typedTerm;
        try {
    ...
    public int transFirstOpInferIndex(Term typedTerm, Boolean reverse) {
        ...
        while (iter!=ultInf){
            ...
            { // ((IA)T)T
                ...
                if ( iter instanceof TypedApp && ((TypedApp)iter).inferType=='e' &&
                     iter.des("1", true) instanceof TypedApp && 
                     ((TypedApp)iter.des("1", true)).inferType=='s'
                   ) 
                    iter = iter.des("2", true);
                ultInf = iter;
            }
                ...
                Term expr, Term initialExpr, Term finalExpr) throws TypeVerificationException
    {
        ...
        if(transFirstOpInferIndex(proof,true)!=0 && expr.des("12").equals(formulaBeingProved) ){ 
