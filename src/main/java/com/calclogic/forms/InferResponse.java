...
public class InferResponse extends GenericResponse{
            ...
            Term methodTerm, Boolean valida, Boolean labeled, Term formula, String nTeo, GenericProofMethod objectMethod) 
    {
        ...
        String statement;
        try {
            statement = centeredBlock("$" + clickableST(newFormula, clickable, methodTerm, false, s) + "$");
        }
        catch (Exception e) {
        ...
        try{
            ...
            // Proof of the first sub-case
                ...
                if (typedTerm == null || typedTerm.type()==null || !typedTerm.type().equals(newFormula)){
                    ...
                    statement = centeredBlock("$" + clickableST(newFormula, clickable, new Const("AI"), false, s) + "$");
        ...
        if (isRootTeorem) {
            ...
            try {
                header = "Theorem " + nTeo + ":<br> "+  
                         centeredBlock("$" + clickableST(formula, clickable, methodTerm, isRootTeorem, s) + "$") +
                         "Proof:<br>";
            }
            catch (Exception e) {
                this.setErrorParser1(true);
